function [Priors Mu Sigma cost]=SEDS_Solver_Direction(Priors_0,Mu_0,Sigma_0,Data,options)
%
...
if options.matlab_optimization
    ...
    if isinf(options.cons_penalty)
        [popt,cost] = fmincon(obj_handle, p0,[],[],[],[],[],[],ctr_handle,optNLP);
    else
        [popt,cost] = fminunc(obj_handle, p0,optNLP);
    end
else
